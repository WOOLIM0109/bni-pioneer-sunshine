...
가입 확인 메일이 보이지 않으면 로그인 창의 **가입 확인 메일 다시 받기**에서 가입한 이메일을 입력합니다. 같은 주소의 재발송은 60초 간격으로 요청할 수 있습니다. 스팸함·수신 차단 목록에서 `sunshine@woolimcompany.kr`를 확인하고, 여러 번 받았다면 가장 최근 메일의 링크를 사용하세요. 발송 서비스의 `delivered`는 수신 메일 서버가 메시지를 수락했다는 뜻이며 받은편지함에 표시됐다는 보장은 아닙니다. 계속 확인할 수 없으면 관리자가 발송 기록과 본인 확인 결과를 대조해 해당 계정만 복구합니다. 전체 가입자의 이메일 인증이나 멤버 승인 검사는 끄지 않습니다.

가입만으로 수정 권한이 생기지는 않습니다. 관리자는 가입자 권한 관리에서 이름·이메일·인증 상태를 함께 확인하고, 이름이나 이메일로 검색해 승인할 수 있습니다. 승인 요청을 먼저 표시하며, 아직 연결을 신청하지 않은 새 가입자는 명단에서 이름이 일치하는 사람이 한 명이고 다른 계정에 연결되지 않았을 때만 후보를 미리 선택합니다. 후보가 맞는지 확인하고 **승인 · 저장**을 눌러야 권한이 생깁니다. 동명이인·이름 불일치·이미 연결된 멤버는 관리자가 직접 확인합니다. 기존의 본인 멤버 연결 신청도 계속 사용할 수 있고, 기존 계정도 연결된 멤버가 있으면 명단의 이름을 표시합니다. 연결 전에는 가입 이름을 표시하고, 가입 이름이 없는 승인 대기 계정은 신청한 멤버의 이름을 표시합니다. 이 이름으로 가입자를 검색할 수 있으며, 연결·가입·현재 신청에서 이름을 확인할 수 없는 계정만 **가입 이름 미등록**으로 표시합니다. 표시 이름만으로 권한을 부여하거나 가입 정보를 변경하지 않습니다. 명단을 보는 분은 가입할 필요가 없습니다.

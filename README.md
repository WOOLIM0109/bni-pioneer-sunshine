...
- 멤버 관리: 승인된 본인 정보 수정, 관리자의 명단·권한 관리, 누구나 내보내기
...
처음에는 **처음 가입 → 이메일·비밀번호 입력 → 가입 확인 메일의 링크 누르기**로 가입합니다. 다음부터는 **이메일과 비밀번호로 로그인**합니다. 같은 브라우저에서는 로그인 상태가 유지됩니다. 비밀번호를 잊었거나 이전에 이메일 링크로만 가입했다면 로그인 창의 비밀번호 설정·재설정 기능으로 메일을 받아 새 비밀번호를 정합니다. 이미 로그인한 사용자는 비밀번호 설정 버튼으로 설정할 수 있습니다.

가입만으로 수정 권한이 생기지는 않습니다. 멤버 관리에서 본인의 이름·회사·전문분야를 확인해 연결을 신청하면 관리자가 확인 후 승인합니다. 성함만으로 자동 연결하지 않으므로 동명이인이나 다른 사람 이름을 선택한 신청은 승인 전에 관리자가 판단합니다. 명단을 보는 분은 가입할 필요가 없습니다.
...
현재 `members`의 화면에 표시하는 멤버 정보는 공개 읽기합니다. 감사 이메일을 숨기는 이번 권한 구분과 별개로, 추후 수집되는 실제 **리퍼럴 트리거·좋은 리퍼럴·고객사 명단**을 공개 열에 그대로 저장하면 안 됩니다. HTML에서 숨기는 것으로는 API 조회를 막을 수 없습니다.
